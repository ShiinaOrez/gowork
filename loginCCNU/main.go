...
var (
	...
	ErrGetProfile = errors.New("ErrGetProfile")
	ErrGetToken = errors.New("ErrGetToken")
)
...
func MakeAccountPreflightRequest() (*AccountReqeustParams, error) {
	...
	for _, cookie := range resp.Cookies() {
		...
		if cookie.Name == "routeportal" {
			fmt.Println("routeportal found")
		}
	}
...
func GetToken(jsessionid string, client *http.Client) error {
	request, err := http.NewRequest("GET", "http://one.ccnu.edu.cn/index", nil)
	if err != nil {
		log.Print(err)
		return err
	}
	...
	request.Header.Set("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.109 Safari/537.36")
	// request.Header.Set("Cookie", "routeportal=5fb17c08d7829466342e4fe60bd21884; JSESSIONID="+jsessionid)
	...
	request, err = http.NewRequest("GET", "http://one.ccnu.edu.cn/index", nil)
	request.Header.Set("Cookie", resp.Header["Set-Cookie"][0])
	fmt.Println(client.Jar)
	return nil
}

func GetProfile(client *http.Client) error {
	request, err := http.NewRequest("POST", "http://one.ccnu.edu.cn/user_portal/index", nil)
	if err != nil {
		log.Print(err)
		return err
	}
...
func LoginRequest(Sid, Password string) error {
	...
	if err := GetToken(params.JSESSIONID, &client); err != nil {
		return ErrGetToken
	}
	if err := GetProfile(&client); err != nil {
